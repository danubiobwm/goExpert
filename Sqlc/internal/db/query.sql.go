...
const createCouse = `-- name: CreateCouse :exec
INSERT INTO courses (id, name, description, category_id, price)
VALUES (?, ?, ?, ?, ?)
`

type CreateCouseParams struct {
	ID          string
	Name        string
	Description sql.NullString
	CategoryID  string
...
func (q *Queries) CreateCouse(ctx context.Context, arg CreateCouseParams) error {
	_, err := q.db.ExecContext(ctx, createCouse,
		arg.ID,
		arg.Name,
		arg.Description,
		arg.CategoryID,
